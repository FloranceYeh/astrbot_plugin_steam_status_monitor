...
## 更新记录
- V3.1.2（2026/07/04）
  - **Bug 修复**：成就推送仅显示图片，不再附带文字消息
  - **Bug 修复**：排行榜 (rank/allrank) 封面获取日期键与数据聚合对齐（凌晨4点边界），修复封面不显示
  - **Bug 修复**：排行榜 (rank/allrank) 新增 Steam 头像框渲染（此前参数未传入导致不显示）
  - **Bug 修复**：玩家切换游戏时（未退出即启动下一款），上一款游戏游玩时长不再丢失

- V3.1.1（2026/07/04）
  - **头像框系统**：开始游戏/结束游戏/list/rank 图片均支持显示 Steam 头像框，异步抓取不影响主进程
  - **缓存配置化**：头像/头像框/封面缓存时间可在 WebUI 配置，默认头像1天/头像框7天/封面永不
  - **alllist 图片版**： steam alllist 改为图片渲染，显示群号+SteamID+下次轮询+竖版封面
  - **权限分级**：新增 permission_level 配置（1=管理员限定 2=查询指令放开 3=开关+添加ID放开）
  - **初始化去重**：修复重启后跨群玩家被重复推送开始游戏通知的问题
  - 修复： get_today_superpower 方法丢失、_check_perm 权限检查无效等问题
