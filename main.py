...
class SteamStatusMonitorV3(Star):
    ...
    async def notify_new_achievements(self, group_id, steamid, player_name, gameid, game_name, new_achievements):
        ...
        # 文本消息（受 notify_send_text 开关控制）
        text_msg = None
        if send_text:
            text_msg = f"🎉 {player_name} 在 {game_name} 中解锁了新成就!\n"
            for achievement in achievements_to_notify:
                text_msg += f"• {achievement}\n"
            if extra_count > 0:
                text_msg += f"...以及另外 {extra_count} 个成就"
        # 按开关组装消息链并推送
        if not tmp_path and not text_msg:
            return  # 两个开关都关则不发
        for session in notify_sessions:
            try:
                msg_chain = []
                if text_msg:
                    msg_chain.append(Plain(text_msg))
                if tmp_path:
